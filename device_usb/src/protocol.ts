import {
  ...
  type UacUpdateResult,
  CommandId,
} from "./types.js";

// ─── BCD Encoding ────────────────────────────────────────────────────────────

export function toBCD(str: string): number[] {
  const out: number[] = [];
...
export function fromBCD(...bytes: number[]): string {
  let str = "";
...
export function encodeFrame(
  ...
  for (const b of bodyArr) msg[idx++] = b & 0xff;
  return msg;
}

// ─── Frame Decoding ──────────────────────────────────────────────────────────

function readUint16BE(buf: number[], offset: number): number {
  return ((buf[offset] & 0xff) << 8) | (buf[offset + 1] & 0xff);
}

function readUint32BE(buf: number[], offset: number): number {
  let val = 0;
  for (let i = 0; i < 4; i++) {
 ...
 */
export function decodeFrame(buffer: number[]): { frame: WireFrame; consumed: number } | null {
  if (buffer.length < 12) return null;
  if (buffer[0] !== FRAME_HEADER[0] || buffer[1] !== FRAME_HEADER[1]) {
    throw new Error(
      `Invalid frame header: 0x${buffer[0].toString(16)} 0x${buffer[1].toString(16)}`,
    );
  }
...
export function parseDeviceInfo(body: number[]): DeviceInfo {
  const vc: string[] = [];
  let vn = 0;
  for (let i = 0; i < 4; i++) {
    const b = body[i] & 0xff;
    if (i > 0) vc.push(String(b));
    vn |= b << ((3 - i) * 8);
  }
  const sn: string[] = [];
  for (let i = 0; i < 16; i++) {
...
export function parseVersionInfo(body: number[]): VersionInfo {
  ...
  const toInt = (arr: number[]) => {
    let v = 0;
    for (let i = 0; i < arr.length; i++) v |= arr[i] << ((3 - i) * 8);
    return v;
  };
...
export function parseTime(body: number[]): DeviceTime {
  const time = fromBCD(
    body[0] & 0xff,
    body[1] & 0xff,
    body[2] & 0xff,
...
export function parseFileCount(body: number[]): FileCount {
  let c = 0;
  for (let i = 0; i < 4; i++) {
    c |= (body[i] & 0xff) << ((3 - i) * 8);
  }
...
export function parseCardInfo(body: number[]): CardInfo {
  let i = 0;
  const read32 = () => {
    const v =
      ((body[i++] & 0xff) << 24) |
...
export function parseSettings(body: number[]): DeviceSettings {
  return {
    autoRecord: body[3] === 1,
    autoPlay: body[7] === 1,
...
export function parseCommandResult(
  body: number[],
): CommandResult & { rawCode: number; rawBody: string } {
  return {
...
export function parseDeleteFile(body: number[]): DeleteFileResult {
  if (body[0] === 0x00) return { result: "success" };
  if (body[0] === 0x01) return { result: "not-exists" };
  return { result: "failed" };
}

export function parseFirmwareUpgrade(body: number[]): FirmwareUpgradeResult {
  const c = body[0];
  if (c === 0x00) return { result: "accepted" };
  if (c === 0x01) return { result: "wrong-version" };
  if (c === 0x02) return { result: "busy" };
  return { result: "unknown" };
}

export function parseUacUpdate(body: number[]): UacUpdateResult {
  const rst = body[0];
...
export function parseFileList(body: number[]): FileInfo[] {
  ...
  while (i < data.length) {
    ...
    if (/^\d{14}REC\d+\.wav$/i.test(name)) {
      const ts = name.replace(
        /^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})REC.*$/i,
        "$1-$2-$3 $4:$5:$6",
      );
      time = new Date(ts);
    } else if (/^.*\.hda$/i.test(name)) {
      const ts = name.replace(
        /^(\d{2})?(\d{2})(\w{3})(\d{2})-(\d{2})(\d{2})(\d{2})-.*\.hda$/i,
        "20$2 $3 $4 $5:$6:$7",
      );
      time = new Date(ts);
    }
...
export type ParsedResponse =
  ...
  | { type: "versionInfo"; data: VersionInfo }
  | { type: "time"; data: DeviceTime }
  | { type: "setTime"; data: CommandResult }
  | { type: "fileList"; data: FileInfo[] }
  | { type: "fileCount"; data: FileCount }
  | { type: "deleteFile"; data: DeleteFileResult }
  | { type: "firmwareUpgrade"; data: FirmwareUpgradeResult }
  | { type: "firmwareUpload"; data: CommandResult }
  | { type: "settings"; data: DeviceSettings }
  | { type: "cardInfo"; data: CardInfo }
  | { type: "uacUpdate"; data: UacUpdateResult }
  | { type: "commandResult"; data: CommandResult }
  | { type: "unknown"; data: { commandId: number; body: number[] } };

export function parseResponse(commandId: number, body: number[]): ParsedResponse {
  switch (commandId) {
...
export function formatDateForDevice(date: Date): number[] {
...
export function buildSettingsPayload(opts: {
  ...
  if (opts.autoRecord !== undefined) data[3] = opts.autoRecord ? 1 : 2;
  if (opts.autoPlay !== undefined) data[7] = opts.autoPlay ? 1 : 2;
  if (opts.notification !== undefined) data[11] = opts.notification ? 1 : 2;
  return data;
}
