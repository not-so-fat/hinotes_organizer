...
export class HiDockDevice {
    ...
    async listFiles(timeoutSec = 30) {
        if (!this.transport.isConnected)
            throw new Error("Device not connected");
        if (this.fileListAccumulator)
            throw new Error("File list request already in progress");
        const countResp = await this.getFileCount(5);
        if (!countResp || countResp.type !== "fileCount")
            return null;
        ...
        return new Promise((resolve, reject) => {
            ...
            const handler = (frame) => {
                if (frame.commandId !== CommandId.QueryFileList || !this.fileListAccumulator)
                    return false;
    ...
    downloadFile(filename, expectedLength, timeoutSec = 600, onProgress) {
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                this.transport.removeFrameHandler(handler);
                reject(new Error(`downloadFile(${filename}): timeout after ${timeoutSec}s (${received}/${expectedLength} bytes)`));
            }, timeoutSec * 1000);
            const handler = (frame) => {
                if (frame.commandId !== CommandId.TransferFile &&
            ...
            this.transport.onFrame(handler);
            this.transport.sendRaw(CommandId.TransferFile, fnameBody).catch((err) => {
                clearTimeout(timeout);
                this.transport.removeFrameHandler(handler);
    ...
    deleteFile(filename, timeoutSec = 10) {
